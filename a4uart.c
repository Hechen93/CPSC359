...
// Note that MMIO_BASE = 0xFE000000 is the ARM physical address on the Pi 4.
#include "gpio.h"
...
//                  on the Raspberry Pi 4. First, the GPIO pins are set up so
...
void uart_init()
{
    ...
    *GPFSEL1 = r;
    ...
    // pins 14 and 15. Enabling and disabling pullups/pulldowns
    // has changed with the Pi 4, as shown below:

    // Get the current bit pattern of the GPPUPPDN0 register
    r = *GPPUPPDN0;

    // Zero out bits 28-29 and 30-31 in this bit pattern, since
    // these map to GPIO pins 14 and 15. The bit pattern 00
    // disables pullups/pulldowns.
    r &= ~( (0x3 << 28) | (0x3 << 30) );
